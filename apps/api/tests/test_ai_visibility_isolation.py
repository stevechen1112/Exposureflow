"""AI visibility tenant isolation integration tests."""
...
from exposureflow_api.models import AIProbeSet
...
@pytest.mark.asyncio
async def test_ai_probe_sets_not_visible_across_workspaces(
    client: AsyncClient, engine
) -> None:
    token_a, workspace_a, site_a = await _bootstrap_user(client, "ai-a@example.com", "AI A")
    token_b, workspace_b, _site_b = await _bootstrap_user(client, "ai-b@example.com", "AI B")
    ...
    async with session_factory() as db:
        db.add(
            AIProbeSet(
                ...
                site_id=UUID(site_a),
                name="Secret probe set",
                prompts_json=["hidden prompt"],
                surfaces_json=["perplexity"],
            )
        )
        await db.commit()

    own = await client.get(
        "/api/v1/ai-visibility/probe-sets",
