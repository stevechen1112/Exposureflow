"""SERP layer tenant isolation integration tests."""
...
from exposureflow_api.models import SerpSlotTarget
...
@pytest.mark.asyncio
async def test_serp_slot_targets_not_visible_across_workspaces(
    client: AsyncClient, engine
) -> None:
    token_a, workspace_a, site_a = await _bootstrap_user(client, "serp-a@example.com", "SERP A")
    token_b, workspace_b, _site_b = await _bootstrap_user(client, "serp-b@example.com", "SERP B")
    ...
    async with session_factory() as db:
        db.add(
            SerpSlotTarget(
                ...
                current_owner="competitor",
            )
        )
        await db.commit()

    own = await client.get(
        "/api/v1/serp/slot-targets",
